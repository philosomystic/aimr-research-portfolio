...
function selectNode(id){activeN=id;focusN=null;activeT=Number(id.split('.')[0]);if(isMobile())closeMobileSheet();renderAll();syncHash();openModal();announce(nodes[id].title+' AIMR Bridge Card opened.')}function renderAll(){renderGrid();renderBanner();renderBrowser();renderMeta()}
