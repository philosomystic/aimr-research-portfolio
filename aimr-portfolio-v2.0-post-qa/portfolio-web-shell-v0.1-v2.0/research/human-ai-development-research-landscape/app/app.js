...
function setFacet(type,key,label){
  ...
  syncHash();
  if(clearing){
    if(isMobile()&&browser.classList.contains('sheet-open'))closeMobileSheet();
    announce('Field filter cleared.');
    return;
  }
  if(isMobile())openFacetResultsSheet(label);
  else navigateAfterFieldSelection(type==='aimr'?'construct':type,key);
...
function closeMobileSheet(){browser.classList.remove('sheet-open','facet-results');genericBrowserHead.hidden=false;sheetBackdrop.hidden=true;document.body.classList.remove('sheet-active');const b=grid.querySelector('.territory.active-panel');if(b)b.focus()}
function openFacetResultsSheet(label){
  ...
  browser.classList.add('sheet-open','facet-results');
  browser.scrollTop=0;
  requestAnimationFrame(()=>closeSheet.focus({preventScroll:true}));
}
...
function selectNode(id){activeN=id;focusN=null;activeT=Number(id.split('.')[0]);if(isMobile())closeMobileSheet();renderAll();syncHash();openModal();announce(nodes[id].title+' Research Bridge Card opened.')}function renderAll(){
...
function jumpToLandscapeTarget(target){
  ...
  announce(
    ...
    target==='constructs'?'developmental constructs':
